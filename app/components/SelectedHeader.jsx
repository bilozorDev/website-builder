  ...
  return (
    <header style={{ backgroundColor: header?.bgColor || "white" }}>
      <NavWrapper>
        ...
        <PopoverGroup
          className={`hidden lg:flex lg:grow-1 ${
            header.alignment.selected != "center" ? "w-full" : null
          }  lg:justify-${header.alignment.selected} lg:gap-x-12`}
        >
        ...
        <div className="hidden lg:flex lg:flex-none">
          <a
            href={header?.menuItems?.cta?.href}
            className="text-sm font-semibold leading-6"
            style={{ color: textColor }}
          >
            {header?.menuItems?.cta?.text} <span aria-hidden="true">&rarr;</span>
          </a>
        </div>
      </NavWrapper>
      <MobileMenuDialog
